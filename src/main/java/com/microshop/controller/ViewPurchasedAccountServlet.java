...
import com.microshop.model.ThongTin;
...
// Sửa lại lớp ThongTin trong ViewPurchasedAccountServlet.java
...
//            }
//            
            List<DonHangSlotSteam> purchasedOrders1 = donhangslotsteamDAO.getMaNguoiDungDaHoanThanh(user.getMaNguoiDung());
            List<ThongTin> purchasedAccounts1 = new ArrayList<>();
            for(DonHangSlotSteam x : purchasedOrders1){
                int MaTaiKhoan = x.getMaTaiKhoanSteam();
                String TenGame = gamesteamDAO.getById(x.getMaGameSteam()).getTenGame();
                TaiKhoanSteam tk = taikhoansteamDAO.getById(x.getMaTaiKhoanSteam());
                String TenDangNhap = tk.getTenDangNhapSteam();
                String MatKhau = tk.getMatKhauSteam();
                purchasedAccounts1.add(new ThongTin(MaTaiKhoan, TenGame, TenDangNhap, MatKhau));
            }
...
//            int cnt1 = 0;
            ...
            request.setAttribute("steamAccounts", purchasedAccounts1);
